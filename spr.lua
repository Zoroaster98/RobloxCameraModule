...
do
	...
	function spr.target(instance: Instance, dampingRatio: number, frequency: number, properties: {[string]: any})
		...
		local state = springStates[instance] -- Throw instance onto table to manage  
		if not state then
			state = {}
			springStates[instance] = state
		end

		for propName, propTarget in properties do -- loop through property goal and Name
			local propValue
			local override = PSEUDO_PROPERTIES[propName] --over ride if proerty amongus
			if override and instance:IsA(override.class) then
			...
			local spring = state[propName] --throw property onto already existing instance table
			if not spring then
